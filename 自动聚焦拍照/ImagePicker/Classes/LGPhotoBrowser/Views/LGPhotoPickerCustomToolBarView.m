...
- (instancetype)initWithFrame:(CGRect)frame showType:(LGShowImageType)showType
{
    ...
    if (self) {
        ...
        if ([[UIToolbar class] respondsToSelector:@selector(appearance)]) {
            [_toolbar setBackgroundImage:nil forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsDefault];// uibarmecompa
...
//        CGSize textSize = [text length] > 0 ? [text sizeWithFont:fileSizeButton.titleLabel.font] : CGSizeZero;
        NSDictionary *attribute = @{NSFontAttributeName: fileSizeButton.titleLabel.font};
        CGSize textSize = [text length] > 0 ? [text sizeWithAttributes:attribute] : CGSizeZero;
...
- (UIBarButtonItem *)createSelecteOriginalItemWithImg:(UIImage *)image
    ...
    NSDictionary *attribute = @{NSFontAttributeName: button.titleLabel.font};
